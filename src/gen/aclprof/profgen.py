...
def Check(flag):
    ...
    for sym in origin:
        ...
        if cnt == 1 :    
            ...
            if flag ==2:
                # load so_func
                print("    ACL_LDSYM(so_aclprof_handle, {:s});".format(func))
...
constructor="""
__attribute__((constructor)) void aclprof_init(){
    void *so_aclprof_handle = dlopen(ACLPROF_LIBFILE,RTLD_GLOBAL | RTLD_NOW);
    if(so_aclprof_handle == NULL){
        fprintf(stderr, "error for open the acl prof lib (file %s), error msg %s\\n", ACLPROF_LIBFILE, dlerror());
        exit(1);
    }
    load_all_prof_funcs(so_aclprof_handle);
}
"""
...
aclgrphProfGraphSubscribe
aclgrphProfGraphUnSubscribe
aclprofGetGraphId
aclprofGetOpAttriValue
aclprofGetOpFlag
aclprofGetSupportedFeatures
aclprofSetCategoryName
aclprofSetStampCategory
aclprofSetStampPayload
*/
"""
def add_notes():
    print(notes)
...
def main():
    ...
    print("void load_all_prof_funcs(void *so_aclprof_handle){")
